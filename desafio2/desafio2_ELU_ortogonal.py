"""
Autores: Gabriel Valentin e Pedro de Colla
...
def ativacao(x: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.elu(x)          # elemento a elemento, sem parâmetros


# E[f(z)²] com z ~ N(0,1): estimado UMA vez, por Monte Carlo. Só depende de f.
_g = torch.Generator().manual_seed(0)
_E_f2 = ativacao(torch.randn(1_000_000, generator=_g)).pow(2).mean().item()

_ATENUACAO_RASA = 0.85      # n_camadas <= 8
_ATENUACAO_FUNDA = 0.70     # n_camadas  > 8
_LIMIAR_PROFUNDIDADE = 8
...
def inicializar(W: torch.Tensor, b: torch.Tensor,
    ...
    if camada == 1:
        # entra o dado padronizado (E[x²] = 1), não f(z): o ganho é 1
        ganho = 1.0
    else:
        # entra f(z): ganho calibrado, atenuado conforme a profundidade
        atenuacao = _ATENUACAO_RASA if n_camadas <= _LIMIAR_PROFUNDIDADE else _ATENUACAO_FUNDA
        ganho = atenuacao * math.sqrt(1.0 / _E_f2)
