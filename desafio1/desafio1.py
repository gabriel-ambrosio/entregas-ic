"""
Autor: Gabriel Ambrosio Valentin - 12411BCC003
...
class Submissao:
   DIM_MAX = DIM_MAX
   GANHOS = (1.0, 2.0, 4.0)                         # banco multi-escala de larguras

   def fit(self, X: torch.Tensor) -> None:
       """Padroniza e escolhe centros entre os pontos de treino, em várias larguras."""
       self.mu, self.sd = X.mean(0), X.std(0) + 1e-8
       Xs = (X - self.mu) / self.sd

       h = self.DIM_MAX                             # d' = 64 > d sempre
       g = torch.Generator().manual_seed(0)         

       # escala base: mediana das distâncias entre pontos
       d2 = torch.cdist(Xs[:300], Xs[:300]) ** 2
       self.sigma = math.sqrt(d2[d2 > 0].median().item() / 2) + 1e-8

       # centros = subconjunto aleatório do treino
       idx = torch.randperm(len(Xs), generator=g)[:h]
       if len(idx) < h:                             # menos pontos que centros: recicla
           idx = idx[torch.arange(h) % len(idx)]
       self.C = Xs[idx].clone()

       # cada centro com sua largura, de acentuado (ganho 1) a suave (ganho 4);
       # o perceptron escolhe as bandas que servem a cada tarefa (cada tarefa tem seu ganho)
       self.ganhos = torch.tensor(self.GANHOS)[torch.arange(h) % len(self.GANHOS)]

   def phi(self, X: torch.Tensor) -> torch.Tensor:
       """(n, d) -> (n, 64): kernel RBF IMQ 
       IMQ = 1/sqrt(1 + u^2), com u = ganho * ||x - c|| / sigma."""
       Xs = (X - self.mu) / self.sd
       u = self.ganhos * torch.cdist(Xs, self.C) / self.sigma
       return (1.0 + u ** 2).rsqrt()
